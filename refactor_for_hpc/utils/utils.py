...
def sample_new_training_points_via_IF(
    ...
    if return_top_k is None:
        ...
        if top_k_kind == "abs":
            top_k_idx = torch.argsort(
                torch.abs(influences).sum(axis=0), descending=True
            )[:return_top_k]
        # TODO: check if pos and neg are correctly aligned
        elif top_k_kind == "pos":
            top_k_idx = torch.argsort(influences.sum(axis=0), descending=True)[
                :return_top_k
            ]
        elif top_k_kind == "neg":
            top_k_idx = torch.argsort(influences.sum(axis=0), descending=False)[
                :return_top_k
            ]
        else:
            raise ValueError(f"Invalid top_k_kind: {top_k_kind}")

        return oversampled_data.train_x_all[top_k_idx]
