...
#SBATCH --job-name=burgers_pretrain
...
ARGS=$(sed -n "$((SLURM_ARRAY_TASK_ID))p" batch_pretrain.txt)
...
apptainer run --bind ${LOCAL_JOB_DIR}:/opt/model_zoo --bind ${HOME}/pinnfluence_finetuning/pinnfluence:/opt/code /home/fe/krasowski/pinnfluence_finetuning/deepxde_generic_code.sif  /opt/code/burgers/pretrain_model.py $ARGS
