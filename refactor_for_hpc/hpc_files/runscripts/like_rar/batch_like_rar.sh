...
ARGS=$(sed -n "$((SLURM_ARRAY_TASK_ID))p" batch_like_rar.txt)
...
apptainer run --bind ${LOCAL_JOB_DIR}:/opt/model_zoo --bind ${HOME}/pinnfluence_finetuning/pinnfluence:/opt/code --bind ${HOME}/pinnfluence_finetuning/model_zoo_src/:/opt/model_zoo_src /home/fe/krasowski/pinnfluence_finetuning/deepxde_generic_code.sif  /opt/code/burgers/like_rar.py $ARGS
