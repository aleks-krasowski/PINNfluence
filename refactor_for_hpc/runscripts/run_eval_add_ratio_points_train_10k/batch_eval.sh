...
#SBATCH --job-name=burgers_sampling
...
#SBATCH --array=412,438,312,320,328,346,354,380,388,404,206,232,256,264,282,290,298,76,102,110,118,144,172,176,198,2,6,10,18,26,620,536
...
#ARGS=$(sed -n "$((SLURM_ARRAY_TASK_ID))p" batch_eval.txt)
ARGS=$(sed -n "$((SLURM_ARRAY_TASK_ID))p" batch_low_ratios.txt)
...
apptainer run --bind ${LOCAL_JOB_DIR}:/opt/model_zoo --bind ${HOME}/pinnfluence_finetuning/pinnfluence:/opt/code --bind ${HOME}/pinnfluence_finetuning/model_zoo_src/:/opt/model_zoo_src /home/fe/krasowski/pinnfluence_finetuning/deepxde_generic_code.sif  /opt/code/burgers/eval_finetuning.py $ARGS
