...
if setting == "sample_new_points":
    ...
    for ratio in ratios:
        ...
        model.train(
            ...
            callbacks=[
                BestModelCheckpoint(
                    f"{DEFAULTS['save_path']}/{checkpoint_filename}/finetuned_RAR_{setting}_{ratio}_test.pt",
                    verbose=1,
                    save_better_only=True,
                ),
                BestModelCheckpoint(
                    f"{DEFAULTS['save_path']}/{checkpoint_filename}/finetuned_RAR_{setting}_{ratio}_train.pt",
                    verbose=1,
                    save_better_only=True,
                    monitor="train loss",
                ),
        ...
        model.save(
            f"{DEFAULTS['save_path']}/{checkpoint_filename}/finetuned_RAR_{setting}_{ratio}_full_epochs"
        )
        ...
        metrics_df.to_csv(
            f"{DEFAULTS['save_path']}/{checkpoint_filename}/metrics_RAR.csv"
        )
